...
import { Bot, BotStore, Flow, Template } from "./DTO";
  ...
  persist(
    (set) => ({
      ...
      addTemplate: (name) => set((state) => {
        ...
        return {
          botTemplates: [...state.botTemplates,newTemplate]
        }
      }),
      ...
      deleteBot: (botId) =>
        set((state) => ({
          ...
          selectedBot:
            state.selectedBot?.id === botId ? null : state.selectedBot,
        })),
      ...
      updateFlow: (botId, flowId, updates) =>
        set((state) => {
          const updatedBots = state.bots.map((bot) => {
            if (bot.id === botId) {
          ...
          const updatedBots = state.bots.map((bot) => {
            if (bot.id === botId) {
              return {
                ...bot,
                flows: bot.flows - 1,
                flowDetails: bot.flowDetails.filter(
                  (flow) => flow.id !== flowId
                ),
                lastUpdated: getCurrentTimestamp(),
              };
      ...
      // Flow Node Actions
      updateFlowNodes: (botId, flowId, nodes, edges = []) =>
        set((state) => {
          const updatedBots = state.bots.map((bot) => {
            if (bot.id === botId) {
              const updatedFlows = bot.flowDetails.map((flow) => {
